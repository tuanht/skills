...
- **Project rules win.** Anything in `CLAUDE.md` or `.claude/docs/` beats a general principle. Existing pattern in surrounding code beats a general principle too — match neighbours, don't refactor toward purity uninvited.
- **Principles conflict; KISS breaks tie.** DRY pushes toward abstraction, KISS toward plain duplication. Two similar blocks not automatically duplication — extract only when logic same *reason to change*, not same shape. Same for SOLID: don't add interface, factory, or layer without concrete second implementation or test seam needing it (YAGNI).
- **Scope.** Principle violation in code the task touch = finding. Pre-existing violation nearby = note only, not fix — Builder stays in scope, Reviewer files it as observation, never blocker.
- **Severity.** Principle findings are style/correctness tier (loop back to Builder) unless they force a design decision — then escalate. Reviewer labels them own judgement, not rule-doc citation, since no project doc backs them.
...
One file per role in `../templates/`. Read the file and use its content verbatim
as the sub-agent prompt, filling the `[BRACKETED]` parts. Templates live outside
this doc so a prompt can be edited without touching role definitions, and so the
...
Every sub-agent role — all of them, no exception — has no channel to user. Question directed at user never answered, sub-agent hangs. Therefore:
